<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@500&display=swap" rel="stylesheet">   
    <link rel="icon" href="../img/logo2.png">
    <link rel="stylesheet" href="../styles/vote.css">
    <title>CinIF</title>
</head>
<body>
    <div class="flex">

        <table class="header">
            <tr>
                <td class="bord"><img src="../img/logo2.png" alt="logo" class="logo"></td>
                <td><p class="titulo">CinIF</p></td>
                <td class="navbg">
                    <table>
                    <tr>
                    <td class="bord"><a href="../back/voteg.php"><button class="navop">Gênero</button></td>
                    <td class="bord"><a href="../back/votef.php"><button class="navops">Filme</button>
                    <div class="seta"></div>
                    </td>
                    <td class="bord"><a href="../back/catalogo.php"><button class="navop">Catálogo</button></td>
                    </tr>
                    </table>

                </td>
            </tr>
        </table>
        <div class="balao">
            <p class="balaot">O filme no topo da lista será exibido, vote na sua preferência e fique atênto ao local e horário do evento</p>
        </div>
            <p class="descm">O filme no topo da lista será exibido, vote na sua preferência</p>
            <img src="../img/banner_m.png" class="bannerm" alt="banner">
    </div>

    <img src="../img/banner_d.png" class="bannerd" alt="banner">
    <p class="descd">O filme no topo da lista será exibido, vote na sua preferência</p>
</body>
</html>